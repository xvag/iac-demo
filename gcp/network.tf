...
module "n-vpc" {
  source        = "./modules/vpc"
  vpc_name      = "${var.vpc.nagios.name}-vpc"
  subnet_name   = "${var.vpc.nagios.name}-subnet"
  subnet_region = var.vpc.nagios.region
  subnet_cidr   = var.vpc.nagios.cidr
}
...
module "fw-nagios-vpc" {
  source     = "./modules/firewall"
  fw_name    = "fw-nagios-vpc"
  vpc_name   = module.n-vpc.vpc_name
  vpc_subnet = module.n-vpc.subnet_name
  rules      = var.fw.nagios_vpc
  src_ranges = ["0.0.0.0/0"]
}
